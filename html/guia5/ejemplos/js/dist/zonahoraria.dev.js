...
var fechaHoy = new Date(); //Cálculo del desfase en el huso horario para hacer
//luego el ajuste con la zona horaria seleccionada
//en el elemento select del formulario

var desfase = -Math.round(fechaHoy.getTimezoneOffset() / 60) + 12; //Registrar evento click del ratón al presionar botones de envío
...
function getHoraLocal() {
  var fechaHoy = new Date(); //Ajustar el desfase horario respecto a la zona seleccionada.
  ...
  var zh = document.zonahoraria.zonas.selectedIndex - desfase; //Sumar (o restar) las horas de desfase respecto a la hora
  //local del cliente

  fechaHoy.setHours(fechaHoy.getHours() + zh); //Informar la hora local del huso horario elegido
